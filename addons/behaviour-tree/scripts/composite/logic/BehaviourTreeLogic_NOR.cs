...
public class BehaviourTreeLogic_NOR : BehaviourTreeCompositor {
    ...
    protected override TickResult ProcessChildren(BehaviourTreeRoot root, Dictionary<string, object> blackboard, float deltaTime, Node[] children){
        ...
        // NOR means if any succeed, fail, but if all children fail, succeed
        return (countSucceeded == 0) ? TickResult.SUCCESS : TickResult.FAILURE;
    }
